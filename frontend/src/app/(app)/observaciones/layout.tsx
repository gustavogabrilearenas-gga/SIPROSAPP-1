...
export default function ObservacionesLayout({ children }: { children: React.ReactNode }) {
  const pathname = usePathname()
  const { user } = useAuth()
  const canSupervise = isSuperuser(user) || isSupervisor(user)

  const tabs = [
    { href: '/observaciones/crud', label: 'CRUD' },
    ...(canSupervise ? [{ href: '/observaciones/supervision', label: 'Supervisión' }] : []),
  ]
