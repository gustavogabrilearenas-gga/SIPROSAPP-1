...
function LotesContent() {
  ...
  const handleLoteAction = async (lote: LoteListItem, action: LoteActionType) => {
    ...
    try {
      ...
      console.log('Error en handleLoteAction:', err);
      const { status, message, details } = handleApiError(err);
      console.log('Error procesado:', { status, message, details });

      if (status === 400) {
        showError('Acción no permitida');
      } else if (status === 422) {
        // Obtener el mensaje directamente del error
        let errorMessage = '';
        if (err.response?.data?.details?.error) {
        ...
        showError('Error', message ?? 'La acción no se puede realizar en el estado actual del lote.');
      }
    } finally {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-gray-600">Gestión completa de la producción</p>
            </div>
          </div>
